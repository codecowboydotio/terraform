...
# This example shows using an SSL certificate from the /Common partition within the AS3 declaration.
# Manage this carefully if you want to do this.
...
data "template_file" "bigiq" {
  template = file("4-bigip_ssl_common.json")
  vars = {
...
resource "bigip_bigiq_as3" "task-4" {
...
resource "bigip_do"  "do-example-4" {
  do_json = data.template_file.do_bigiq.rendered
  timeout = 5

  depends_on = [ bigip_bigiq_as3.task-4 ]
}
